...
function woo_ce_get_product_vendor_assoc_orders( $vendor_ids = array() ) {
    ...
    if ( woo_ce_detect_export_plugin( 'yith_vendor' ) ) {
        ...
        global $wpdb;
        $sql       = 'SELECT `order_id` FROM `' . $wpdb->prefix . 'yith_vendors_commissions` WHERE `vendor_id` IN (' . implode( ',', array_fill( 0, count( $vendor_ids ), '%d' ) ) . ') LIMIT 1';
        $order_ids = $wpdb->get_col( $wpdb->prepare( $sql, ...$vendor_ids ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
