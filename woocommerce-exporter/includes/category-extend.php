...
if( is_admin() ) {
	...
	function woo_ce_categories_filter_by_language() {
		...
		ob_start(); ?>
<p><label><input type="checkbox" id="categories-filters-language" /> <?php esc_html_e( 'Filter Categories by Language', 'woocommerce-exporter' ); ?></label></p>
<div id="export-categories-filters-language" class="separator">
	<ul>
		<li>
<?php if( !empty( $languages ) ) { ?>
			<select data-placeholder="<?php esc_html_e( 'Choose a Language...', 'woocommerce-exporter' ); ?>" name="category_filter_language[]" multiple class="chzn-select" style="width:95%;">
...
<?php } ?>
	...
	<p class="description"><?php esc_html_e( 'Select the Language\'s you want to filter exported Categories by. Default is to include all Language\'s.', 'woocommerce-exporter' ); ?></p>
</div>
...
function woo_ce_extend_cron_category_dataset_args( $args, $post_id, $export_type = '', $is_scheduled = 0 ) {
...
add_action( 'woo_ce_extend_cron_dataset_args', 'woo_ce_extend_cron_category_dataset_args', 10, 4 );
...
function woo_ce_extend_category_item( $category ) {
	...
	if( is_multisite() ) {
		...
		$current_blog_details = get_blog_details( array( 'blog_id' => $category->blog_id ) );
		$category->blog_name = $current_blog_details->blogname;
